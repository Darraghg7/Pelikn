...
export async function fetchCleaningTasks(venueId: string): Promise<CleaningResult> {
  const [{ data: tData, error: tErr }, { data: cData, error: cErr }] = await Promise.all([
    supabase.from('cleaning_tasks').select('id, title, frequency, assigned_role, role_id, is_active, venue_id, created_at').eq('venue_id', venueId).eq('is_active', true).order('title'),
    supabase
