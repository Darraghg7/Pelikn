...
// ── Splash screen ────────────────────────────────────────────────────────────
// The actual animated splash markup lives in index.html so it exists before
// React and lazy chunks load. This component only controls native handoff,
// landing timing, and cleanup.

function SplashScreen() {
  React.useEffect(() => {
    // Splash is already started by the inline <script> in index.html that runs
    // immediately after #pk-splash is inserted into the DOM — no waiting for
    // React. This call is a belt-and-suspenders safety net only (idempotent).
    window.__peliknStartSplash?.()

    // Hide native Capacitor splash as fire-and-forget — DO NOT await this
    // before calling __peliknStartSplash. The native bridge round-trip takes
    // 100–300ms on iOS; chaining them caused a blank green screen gap.
