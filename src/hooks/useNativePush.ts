...
export async function registerNativePush(staffId: string, venueId: string, navigate: ((url: string) => void) | null): Promise<void> {
  ...
  try {
    ...
    if (!listenersRegistered) {
      ...
      await PushNotifications.addListener('pushNotificationActionPerformed', ({ notification }) => {
        const url = (notification?.data as { url?: string })?.url
        if (url && navigate) {
          navigate(url)
        }
      })
...
export async function unregisterNativePush(staffId: string): Promise<void> {
