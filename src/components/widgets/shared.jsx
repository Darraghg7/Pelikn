...
export function WidgetShell({ title, badge, to, linkLabel, aside, flush = false, status, children }) {
  ...
  return (
    <div className="bg-white dark:bg-paperDark rounded-2xl border border-line dark:border-white/10 overflow-hidden h-full flex flex-col">
      <div className={`flex items-center justify-between gap-3 px-4 sm:px-4 pt-3 shrink-0 ${flush ? 'pb-3 border-b border-line dark:border-white/10' : 'pb-2'}`}>
      ...
      <div className={`flex-1 ${flush ? '' : 'px-4 sm:px-4 pb-4'}`}>{children}</div>
    </div>
  )
}
...
export function BigNumber({ value, label, alert }) {
  return (
    <div className="py-1">
      <p className={`font-mono text-[28px] leading-tight font-semibold ${alert ? 'text-bad dark:text-[#f19a86]' : 'text-ink dark:text-white'}`}>{value}</p>
...
export function MiniRow({ label, value, warn, good }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-1.5">
      ...
      <span className={`font-mono text-[14px] font-semibold ${tone}`}>{value}</span>
    </div>
  )
}
