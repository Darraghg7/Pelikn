      ...
      {loading ? (
        ...
        <div className="divide-y divide-line dark:divide-white/10">
          {shifts.slice(0, 5).map(s => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-3 px-4 sm:px-4 py-2.5">
                <span className="relative shrink-0 w-10 h-10 rounded-full bg-brand-tint dark:bg-white/10 inline-flex items-center justify-center text-[14px] font-semibold text-ink dark:text-white">
                  {letters.toUpperCase()}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[15px] font-semibold text-ink dark:text-white truncate">{name}</p>
          ...
          {shifts.length > 5 && (
            <p className="text-[13px] text-ink3 dark:text-white/45 px-4 sm:px-4 py-3">+{shifts.length - 5} more on the rota</p>
          )}
        </div>
      )}
    </WidgetShell>
