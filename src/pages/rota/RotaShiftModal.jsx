    ...
    >
      {modal && (
        <div className="flex flex-col gap-5">
          ...
          {modal.dayShifts?.length > 0 && (
            <div className="rounded-xl border border-warning/30 bg-warning/6 p-3 flex flex-col gap-2">
              ...
              {modal.dayShifts.map((sh) => (
                <div key={sh.id} className="flex items-center justify-between bg-white dark:bg-paperDark rounded-lg px-3 py-2.5 border border-charcoal/8 dark:border-white/8">
                  <div>
                    <p className="font-semibold text-charcoal dark:text-white text-sm font-mono">
                      {sh.start_time?.slice(0,5) ?? ''} – {sh.end_time?.slice(0,5) ?? ''}
                      <span className="font-sans font-normal text-charcoal/40 dark:text-white/35 text-xs ml-2">
          ...
          {/* Quick presets */}
          <div>
            <p className="text-[11px] tracking-widest uppercase text-charcoal/30 dark:text-white/30 mb-2">Quick Presets</p>
                  ...
                  <button
                    ...
                    className={[
                      ...
                      active
                        ...
                        : 'bg-white dark:bg-paperDark text-charcoal/60 dark:text-white/50 border-charcoal/15 dark:border-white/15 hover:border-charcoal/35 dark:hover:border-white/35 hover:text-charcoal dark:hover:text-white',
                    ].join(' ')}
                  >
                    <p className="font-semibold">{p.label}</p>
                    <p className={`text-[11px] mt-0.5 ${active ? 'opacity-60' : 'text-charcoal/35 dark:text-white/30'}`}>
                      {p.start}–{p.end}
                    </p>
                  </button>
                )
          ...
          {/* Time pickers */}
          <div>
            <p className="text-[11px] tracking-widest uppercase text-charcoal/30 dark:text-white/30 mb-2">Custom Times</p>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] tracking-widest uppercase text-charcoal/40 dark:text-white/35">Start</label>
                <TimeSelect
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] tracking-widest uppercase text-charcoal/40 dark:text-white/35">End</label>
                <TimeSelect
                  value={form.endTime}
            ...
            {duration && (
              <div className="mt-3 rounded-xl bg-charcoal/4 dark:bg-white/5 px-4 py-3 flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  {shiftWage && (
                    <div className="text-right">
                      <p className="text-xs text-charcoal/50 dark:text-white/40">Est. cost (paid hrs)</p>
                      <p className="font-semibold text-charcoal dark:text-white font-mono">{shiftWage}</p>
                    </div>
                  )}
                </div>
                {breakMins > 0 && (
                  <p className="text-[11px] text-charcoal/40 dark:text-white/35 border-t border-charcoal/8 dark:border-white/8 pt-2">
          ...
          {/* Role selector */}
          <div>
            <p className="text-[11px] tracking-widest uppercase text-charcoal/40 dark:text-white/35 mb-2">Role</p>
            {venueRoles.length > 0 ? (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                />
                <p className="text-[11px] text-charcoal/35 dark:text-white/30">Add roles in Settings → Rota Roles to get quick-select chips here.</p>
              </div>
            )}
          </div>
          ...
          {dutyTemplates.length > 0 && (
            <div className="border-t border-charcoal/8 dark:border-white/8 pt-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-sm font-medium text-charcoal dark:text-white">Assign duty?</p>
                  <p className="text-[11px] text-charcoal/40 dark:text-white/35 mt-0.5">Optional task checklist for this shift</p>
                </div>
                <Toggle
                  checked={assignDuty}
                    ...
                    <button
                      ...
                      className={[
                        ...
                        selectedDutyId === t.id
                          ...
                          : 'border-charcoal/12 dark:border-white/15 text-charcoal dark:text-white hover:border-charcoal/30 dark:hover:border-white/30',
                      ].join(' ')}
                    >
                      {t.title}
                      <span className="text-[11px] text-charcoal/35 dark:text-white/30 font-normal ml-2">
