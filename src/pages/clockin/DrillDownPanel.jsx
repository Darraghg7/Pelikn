...
const DrillDownPanel = memo(function DrillDownPanel({
  ...
  return (
    <div className="mx-4 mb-2 rounded-2xl border border-charcoal/8 dark:border-white/8 bg-white dark:bg-paperDark overflow-hidden">
      {activeDays.map((d, i) => {
        const dateStr   = format(d, 'yyyy-MM-dd')
          ...
          <div key={dateStr} className={['px-4 py-3', i > 0 ? 'border-t border-charcoal/6 dark:border-white/8' : ''].join(' ')}>
            ...
            {status === 'absent' && (
              <p className="text-xs text-danger font-medium">
                ...
                {dayShifts.length > 0 && (
                  <span className="text-charcoal/40 dark:text-white/35 font-normal ml-1">
            ...
            {dayData?.sessions.map((s, si) => {
              ...
              return (
                <div key={si} className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 text-xs mb-1">
                  <span className="text-charcoal/40 dark:text-white/35 font-medium">Clock in:</span>
                  <span className="font-mono text-charcoal dark:text-white font-semibold">{fmtTime(s.in)}</span>

                  {s.breaks.filter(b => b.start && b.end).map((b, bi) => (
                    <React.Fragment key={bi}>
                      <span className="text-charcoal/30 dark:text-white/30">·</span>
                      <span className="text-charcoal/40 dark:text-white/35">Break:</span>
                      <span className="font-mono text-charcoal/60 dark:text-white/50">{fmtTime(b.start)}–{fmtTime(b.end)}</span>
                    </React.Fragment>
                  ))}

                  <span className="text-charcoal/30 dark:text-white/30">·</span>
                  <span className="text-charcoal/40 dark:text-white/35 font-medium">Clock out:</span>
                  <span className={['font-mono font-semibold', s.out ? 'text-charcoal dark:text-white' : 'text-charcoal/30 dark:text-white/30 italic'].join(' ')}>
                  ...
                  {sessionWorked !== null && (
                    <>
                      <span className="text-charcoal/30 dark:text-white/30">·</span>
                      <span className="font-mono text-charcoal/60 dark:text-white/50">{formatMinutes(Math.round(sessionWorked))}</span>
                    </>
                  )}
            ...
            {dayShifts.length > 0 && status !== 'absent' && (
              <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-charcoal/45 dark:text-white/40">
                <span>Scheduled:</span>
