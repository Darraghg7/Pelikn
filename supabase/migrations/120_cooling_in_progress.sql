-- ============================================================================
-- 120: Cooling batches can be started before they're finished
--
-- The redesigned Cooling logs page starts a timer when a batch goes into the
-- chiller and records the end temperature when it comes out. The batch has to
-- exist in the database while it cools, so any device in the kitchen can see it
-- and anyone can finish it. Until now every row needed an end temperature.
--
--   - end_temp becomes nullable. NULL = still cooling.
--   - finished_at records when the end temperature was taken, so the page can
--     show how long the cool took (target: 8°C or below within 90 minutes).
--     Existing rows are left NULL: they were logged after the fact, so the
--     time they were entered isn't a reliable finish time.
--   - 'fridge' joins the allowed cooling methods (the new page offers Blast
--     chiller / Ice bath / Fridge / Ambient). Old values stay valid for history.
--   - get_dashboard_snapshot counts finished batches only, so a batch that's
--     still cooling doesn't tick the dashboard's cooling check as done.
--     Full CREATE OR REPLACE — everything but the coolingLogsToday expression
--     and the comment is copied unchanged from 105_onshift_clocked_in.sql.
-- ============================================================================

ALTER TABLE cooling_logs ALTER COLUMN end_temp DROP NOT NULL;
ALTER TABLE cooling_logs ADD COLUMN IF NOT EXISTS finished_at timestamptz;
...
ALTER TABLE cooling_logs ADD CONSTRAINT cooling_logs_cooling_method_check
  CHECK (cooling_method IN ('ambient','ice_bath','blast_chiller','fridge','cold_water','other'));

-- The Log tab lists a venue's batches that are still cooling
CREATE INDEX IF NOT EXISTS cooling_logs_in_progress_idx
  ON cooling_logs (venue_id, started_at) WHERE end_temp IS NULL;
...
COMMENT ON FUNCTION get_dashboard_snapshot(uuid, date, timestamptz, timestamptz, text[]) IS
  ...
  'counts finished batches only, not ones still cooling (120).';
