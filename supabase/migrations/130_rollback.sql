-- Rollback for 130_cleaning_first_cycle.sql.
-- Restores 120's get_dashboard_snapshot and 125's get_manager_notifications_data
-- verbatim: never-done cleaning tasks count as overdue from creation again, and
-- the notifications payload drops created_at (the client then falls back to the
-- old rule for notifications).
