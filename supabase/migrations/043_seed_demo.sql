-- ============================================================================
-- 043: Demo seed function
-- Wipes and rebuilds all data for the demo owner on every login.
-- ============================================================================
...
BEGIN
  ...
  -- ── 1. Delete only the demo venues (leave real venues untouched) ──────────
  ...
  INSERT INTO staff_venue_links (staff_id, venue_id, role) VALUES
    (s1, v2, 'owner'),   -- Sarah manages both
    (s5, v2, 'staff'),   -- Lucy works both
    (s8, v1, 'staff');   -- Conor works both
  ...
  -- Brew & Bloom staff — mostly on time; James is 5 min late most days
  ...
  CROSS JOIN (VALUES
    ...
    (s2, 'clock_in',    INTERVAL  '9 hours 7 minutes'),   -- late
  ...
  -- Corner Cup staff — Conor is late on Mondays
  INSERT INTO clock_events (venue_id, staff_id, event_type, occurred_at)
  ...
  CROSS JOIN (VALUES
    ...
    (s8, 'clock_in',    INTERVAL  '9 hours 10 minutes'),  -- late
  ...
  -- Fridge temp logs — 7 days, am + pm (normal range)
  ...
  -- One out-of-range reading on Prep Fridge 2 days ago (triggers alert)
  INSERT INTO fridge_temperature_logs (venue_id, fridge_id, fridge_name, temperature, logged_by, logged_by_name, logged_at)
  ...
  -- Cleaning completions — daily tasks done for past 6 days, weekly done this week
  ...
  -- Weekly cleaning done at start of current week
  INSERT INTO cleaning_completions (venue_id, cleaning_task_id, completed_by_staff_id, completed_by_name, completed_at)
  VALUES
  ...
  -- ct4 (monthly) has NOT been done recently — will show as overdue
  ...
  -- ct12 (monthly) has NOT been done — will show overdue
  ...
  -- Corner Cup opening/closing (reuse same pattern, different venue)
  ...
  -- Task completions for past 6 days
  ...
  RETURNING id INTO si1;  -- only captures last; we don't need individual IDs
  ...
  VALUES
    (v1, 'temperature', 'Prep Fridge temperature exceeded 5°C',
     'Prep fridge recorded 6.8°C on morning check. All chilled stock quarantined for inspection.',
    ...
    (v1, 'cleaning', 'Monthly fridge deep clean overdue',
     'Deep clean of all fridges not completed within the 30-day schedule.',
     'Scheduled for this Saturday with full closure of prep area.',
     'major', 'open', 'Sarah Mitchell', (t - interval '1 day')::timestamptz),
    (v2, 'delivery', 'Chilled delivery arrived at 5.1°C — above threshold',
     'Milk delivery from Meadow Fresh Dairy arrived above 5°C. Batch rejected.',
     'Supplier contacted. Replacement delivery arranged.',
     'major', 'resolved', 'Emma Walsh', (t - interval '1 day')::timestamptz),
    (v1, 'pest', 'Mouse droppings found near dry store',
     'Single sighting of droppings in dry goods area near flour bins.',
     'Pest contractor attended same day. Traps placed. No further sightings.',
     'critical', 'resolved', 'Sarah Mitchell', (t - interval '8 days')::timestamptz);

  UPDATE corrective_actions
    SET status = 'resolved', resolved_by = 'Emma Walsh', resolved_at = (t - interval '12 hours')::timestamptz
  WHERE title = 'Chilled delivery arrived at 5.1°C — above threshold' AND venue_id = v2;

  UPDATE corrective_actions
    SET status = 'resolved', resolved_by = 'Sarah Mitchell', resolved_at = (t - interval '6 days')::timestamptz
  ...
  VALUES
    (v1, 'inspection', 'rodent', 'Kitchen & dry store',
     'Scheduled quarterly pest inspection. No live activity found. Existing traps effective.',
    ...
    (v1, 'sighting',   'rodent', 'Dry store — near flour bins',
     'Mouse droppings found during stock rotation. Approximately 6 droppings.',
    ...
    (v2, 'inspection', 'fly',    'General kitchen area',
     ...
     'Bins moved further from prep area. UV trap installed.',             'low', 'resolved',
     s3, 'Emma Walsh',      (t - interval '10 days')::timestamptz);

  -- ── 23. Staff training ───────────────────────────────────────────────────
  -- Each staff member gets 1–2 certs. Some expired, some expiring soon.
  INSERT INTO staff_training (venue_id, staff_id, title, issued_date, expiry_date, notes)
  VALUES
    -- Sarah: Level 3 Food Hygiene (valid), COSHH (expiring in 20 days)
    ...
    -- James: Level 2 Food Hygiene (valid), First Aid (expired)
    (v1, s2, 'Level 2 Award in Food Safety',  (t - interval '18 months')::date, (t + interval '6 months')::date, null),
    (v1, s2, 'Emergency First Aid at Work',    (t - interval '3 years')::date, (t - interval '5 days')::date, 'Certificate expired — renewal needed'),
    -- Emma: Level 3 (valid), Allergen (valid)
    ...
    -- Lucy: Level 2 (valid), Allergen (expiring in 25 days)
    (v1, s5, 'Level 2 Award in Food Safety',  (t - interval '2 years')::date, (t + interval '8 months')::date, null),
    (v1, s5, 'Allergen Awareness',             (t - interval '1 year')::date, (t + interval '25 days')::date, 'Booked for refresh course'),
    -- Ryan: Level 2 (valid)
    (v2, s6, 'Level 2 Award in Food Safety',  (t - interval '8 months')::date, (t + interval '16 months')::date, null),
    -- Aoife: Level 2 (valid), Customer allergen course (valid)
  ...
  VALUES
    (v1, s4, (t - interval '30 days')::date, 'Sarah Mitchell',
     ...
     'Induction training for new kitchen staff.', 'Sarah Mitchell',
     true, (t - interval '29 days')::timestamptz),
    (v2, s8, (t - interval '14 days')::date, 'Emma Walsh',
     ARRAY['Allergen awareness', 'COSHH', 'Personal hygiene'],
     'Monthly training session.', 'Emma Walsh',
     true, (t - interval '13 days')::timestamptz);
  ...
  SELECT v1,
    ...
    'I have a family commitment this day — can you cover?';
  ...
  SELECT v2,
    ...
    'Could we swap? I have a hospital appointment in the morning.';
  ...
  VALUES
    (v1, s2, 'James O''Brien', (t - interval '3 days')::date, 'manual_override', 7.0, 8.0, 'Forgot to clock out — confirmed with staff');
